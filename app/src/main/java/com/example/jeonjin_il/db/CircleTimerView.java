...
//        canvas.drawColor(Color.RED);
        ...
        float textLength = mNumberPaint.measureText("5");
        canvas.drawText("0", mCx, getMeasuredHeight() / 2 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
                mLongerLineLength + mGapBetweenNumberAndLine + getFontHeight(mNumberPaint), mNumberPaint);
        canvas.drawText("5", mCx + mRadius - mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine - mLongerLineLength -
                textLength / 2
                - mGapBetweenNumberAndLine, mCy + getFontHeight(mNumberPaint) / 2, mNumberPaint);
        canvas.drawText("10", mCx, getMeasuredHeight() / 2 + mRadius - mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine -
                mLongerLineLength - mGapBetweenNumberAndLine, mNumberPaint);
        canvas.drawText("15", getMeasuredHeight() / (float)2.5 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
        ...
        switch (event.getAction() & event.getActionMasked())
        {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mInCircleButton && isEnabled())
                {
                    ...
                    mCurrentTime = (int) (60 / (6 * Math.PI) * mCurrentRadian * 60);
                    invalidate();
                }
                break;
    ...
    public void setCurrentTime(int time)
    {
        if (time >= 0 && time <= 3600)
        {
            ...
            this.mCurrentRadian = (float) (time / 60.0f * 6 * Math.PI / 60);
            invalidate();
        }
    }
